...
impl Config {
    fn merge(self, other: Config) -> Config {
        Config {
            ...
            action_timeout: other.action_timeout.or(self.action_timeout),
...
pub fn parse_flags(args: &[String]) -> Flags {
    ...
    let mut flags = Flags {
        ...
        action_timeout: env::var("AGENT_BROWSER_ACTION_TIMEOUT")
            .ok()
            .and_then(|s| s.parse::<u64>().ok())
            .or(config.action_timeout),
...
mod tests {
    ...
    #[test]
    fn test_parse_action_timeout_flag() {
        let flags = parse_flags(&args("--action-timeout 30000 click @e1"));
        assert_eq!(flags.action_timeout, Some(30000));
    }

    #[test]
    fn test_parse_action_timeout_with_session() {
        let flags = parse_flags(&args("--session mysession --action-timeout 45000 fill @e2 hello"));
        assert_eq!(flags.session, "mysession");
        assert_eq!(flags.action_timeout, Some(45000));
    }

    #[test]
    fn test_clean_args_removes_action_timeout() {
        let cleaned = clean_args(&args("--action-timeout 30000 click @e1"));
        assert_eq!(cleaned, vec!["click", "@e1"]);
    }

    #[test]
    fn test_parse_action_timeout_no_value() {
        let flags = parse_flags(&args("--action-timeout"));
        assert_eq!(flags.action_timeout, None);
    }

    #[test]
    fn test_parse_action_timeout_invalid_value() {
        let flags = parse_flags(&args("--action-timeout notanumber click @e1"));
